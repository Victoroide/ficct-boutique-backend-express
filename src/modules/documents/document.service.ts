...
export class DocumentService {
  ...
  async confirmUpload(documentId: string, recordedBy: string, claimedSha256: string): Promise<DocumentRow> {
    ...
    const updated = await documentRepository.markActiveWithHash(documentId, actualSha, head.size, head.contentType ?? doc.mime_type);
  ...
  async download(documentId: string): Promise<{ url: string; expiresIn: number; document: DocumentRow }> {
    const doc = await documentRepository.findById(documentId);
    if (!doc) throw new AppError('NOT_FOUND', 'document not found');
  ...
  async verify(id: string): Promise<{ document: DocumentRow; intact: boolean; chainIntact: boolean; storedSha: string; currentSha: string; brokenAt: number }> {
    const doc = await documentRepository.findById(id);
    if (!doc) throw new AppError('NOT_FOUND', 'document not found');
